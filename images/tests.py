...
def create_temporary_image():
    ...
    test_image_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "test-assets", "cat.jpg"
    )
    ...
    return InMemoryUploadedFile(
        image_file, None, "cat.jpg", "image/jpeg", image_file.tell, None
    )
...
class ImageModelTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.exptected_image_name = "cat"
        cls.exptected_ext = ".jpg"
        cls.exptected_image_path = cls.image.get_original_url
    ...
    def test_image_str_method(self):
        expected_str = (
            f"{str(self.user)} - {self.exptected_image_path.rsplit('/media/')[1]}"
        )
        self.assertEqual(str(self.image), expected_str)

    def test_get_original_url_method(self):
        self.assertEqual(self.image.get_original_url, self.exptected_image_path)
...
class ImageAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.user = get_user_model().objects.create_user(
            username="testuser", password="testpass", account_tier=enterprise_account
        )
    ...
    def test_get_expiring_link_detail_with_expired_link(self):
        ...
        expiring_link = ExpiringLink.objects.create(
            image=image, link="abc", expires_in=-3600
        )
        url = reverse(
            "expiring-link-detail", kwargs={"signed_link": expiring_link.link}
        )
        response = self.client.get(url)
